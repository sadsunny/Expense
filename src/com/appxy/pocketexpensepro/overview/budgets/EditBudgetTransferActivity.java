...
public class EditBudgetTransferActivity extends BaseHomeActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		inflater = LayoutInflater.from(EditBudgetTransferActivity.this);
		...
		amountString = (String) mList.get(0).get("amount");
		
		from_id = (Integer) mList.get(0).get("fromBudget");
		to_id = (Integer) mList.get(0).get("toBudget");
		...
		if (mBudgetList != null && mBudgetList.size() > 0) {
			...
			for (Map<String, Object> iMap : mBudgetList) {
				int tId = (Integer) iMap.get("_id");
				if (tId == from_id) {
					fromSelected = i;
					fromButton.setText((String) iMap.get("categoryName"));
				}
				if (tId == to_id) {
					toSelected = i;
					toButton.setText((String) iMap.get("categoryName"));
				}
				i++;
			}
		...
		amountEditText.setSelection(MEntity.doubl2str(amountString).length());
		...
		public void onClick(View v) {
			...
			case R.id.action_done:
				...
				if (amountDouble == 0) {
					...
					new AlertDialog.Builder(EditBudgetTransferActivity.this)
							.setTitle("Warning! ")
							.setMessage(
			...
			case R.id.from_btn:
				...
				fromListViewAdapter = new ChooseTypeListViewAdapter(
						EditBudgetTransferActivity.this);
